...
1. **改完必驗**:`npm run build`(tsc strict)綠 + `npm run smoke` 全項全綠才算完成(目前 233 項)。新功能要加對應 smoke 步驟。
...
WASD 移動|空白鍵 跳(風語石:二段/三段跳、按住滑翔)|Shift 閃避|左鍵短按普攻/按住集氣迴旋斬(滿氣加衝擊波)|Q 舉盾|數字 1–6 施放裝備中的主動寶石技能(火焰斬/地震波/冰箭/瞬移/熔岩噴發/碧波震盪/生命汲取/星芒斬/楓刃旋舞/幽影迴環;鍵位於背包點寶石下方數字 1–6 指定)|Z 連鎖閃電(雷光果,麻痺)|T 引力漩渦(引力果,聚怪)|G 星隕雨(星辰果,多目標)|R 藥水|F 對話/上下船/潛入/設置重生點|Tab 背包|M 地圖(當前海域島嶼 3D 迷你模型;點島進上帝視角觀察,單機全暫停、多人只暫停自己,M 回地圖/ESC 返回)|ESC 設定

> **兩套能力系統**:① 靈紋寶石(`gems.ts`/`GemBag`)= 群島刻入之力,十顆主動 + 風語/潮汐被動;主動寶石綁定數字鍵 1–6(`GemBag.slots`,背包 `assignSlot` 指定/對調、`ensureSlots` 補齊,存檔 `gemSlots`),主迴圈 `gemCast(key)` 依綁定鍵位施放。② 靈樹果實(`fruits.ts`/`FruitBag`)= 世界樹原初之力,三顆,Z/T/G。兩者平行:各自存檔欄位、背包區塊、HUD 技能列、鍛造升階。加新果實照寶石模式抄。
...
- 效能:目前全場景常駐(190 隻敵人(含 96 隻沉眠副本敵人)+ 26 島(含祭壇島與六座副本島)),要擴更多島嶼時做分區載入;海面網格只有一張,依玩家所在海域(含兩座副本海域,依 x 切換中心)移動。
- 試煉副本兩座(祭壇島奉獻後面板擇一:靈脈試煉/星砂試煉,`main.ts` DUNGEONS 表)為單機內容:多人連線時祭壇停用;副本進度不入存檔,重整後回祭壇島重新奉獻。加新副本 = terrain.ts 加三環島 + DUNGEONS 加一筆(mob/boss/難度/獎勵)。
